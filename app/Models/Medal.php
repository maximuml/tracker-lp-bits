...
class Medal extends NexusModel
{
    ...
    public static function listGetTypes($onlyKeyValues = false): array
    {
        ...
        foreach ($results as $type => &$info) {
            $text = Locale::trans("medal.get_types.{$type}", [], null);
            $keyValues[$type] = $text;
            $info['text'] = $text;
        }
        if ($onlyKeyValues) {
    ...
    {
        return Locale::trans('medal.get_types.'.$this->get_type, [], null);
    }

    public function getInventoryTextAttribute(): string
    {
        return $this->inventory !== null ? (string) $this->inventory : Locale::trans('label.infinite', [], null);
    }
    ...
    public function checkCanBeBuy()
    {
        ...
        if ($this->sale_begin_time && $this->sale_begin_time->gt($now)) {
            throw new \RuntimeException(Locale::trans('medal.before_sale_begin_time', [], null));
        }
        if ($this->sale_end_time && $this->sale_end_time->lt($now)) {
            throw new \RuntimeException(Locale::trans('medal.after_sale_end_time', [], null));
        }
        if ($this->inventory !== null && $this->inventory <= 0) {
            throw new \RuntimeException(Locale::trans('medal.inventory_empty', [], null));
        }
    }
    ...
    /** @return  BelongsToMany<User, $this> */
    public function valid_users(): BelongsToMany
    {
