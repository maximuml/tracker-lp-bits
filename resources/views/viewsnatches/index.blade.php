...
@foreach ($rows as $row)
<tr {{ $row['highlight'] }}><td class=rowfollow align=center>{{ $row['usernameHtml'] }}</td>@if ($canViewConfidential)<td class=rowfollow align=center><span class='nowrap'>{{ $row['ip'] }}</span></td>@endif<td class=rowfollow align=center>{{ $row['trafficHtml'] }}</td><td class=rowfollow align=center>{{ $row['ratioHtml'] }}</td><td class=rowfollow align=center>{{ $row['seedtime'] }}</td><td class=rowfollow align=center>{{ $row['leechtime'] }}</td><td class=rowfollow align=center>{{ $row['completedAtHtml'] }}</td><td class=rowfollow align=center>{{ $row['lastActionHtml'] }}</td><td class=rowfollow align=center style='padding: 0px'>{{ $row['reportHtml'] }}</td></tr>
@endforeach
</table>
{{ $pagerbottom ?? '' }}
@else
