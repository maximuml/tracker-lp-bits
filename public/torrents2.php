...
echo '<div class="t2-header">';
echo '<h1>' . ($sectiontype == $browsecatmode ? ($lang_torrents['head_torrents'] ?? 'Torrents') : ($lang_torrents['head_special'] ?? 'Special')) . '</h1>';
echo '<div class="t2-switcher">';
foreach (['table' => 'Table', 'card' => 'Cards', 'compact' => 'Compact'] as $v => $label) {
    $query = $_GET;
    $query['view'] = $v;
    $url = 'torrents2.php?' . http_build_query($query);
    $active = $view === $v ? ' t2-switcher__btn--active' : '';
    printf('<a class="t2-switcher__btn%s" href="%s">%s</a>', $active, $url, $label);
}
echo '</div></div>';
