...
if (! empty(SupportContext::getQuery('h'))) {
...
<tr>
  ...
  <td<?php echo SupportContext::getQuery('n') ? $highlight : ''?>><input name="n" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('n'))?>" size=35></td>

  <td valign="middle" class=rowhead>Ratio:</td>
  <td<?php echo SupportContext::getQuery('r') ? $highlight : ''?>><select name="rt">
<?php
    $options = ['equal', 'above', 'below', 'between'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('rt') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select>
    <input name="r" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('r'))?>" size="5" maxlength="4">
    <input name="r2" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('r2'))?>" size="5" maxlength="4"></td>

  <td valign="middle" class=rowhead>Member status:</td>
  <td<?php echo SupportContext::getQuery('st') ? $highlight : ''?>><select name="st">
<?php
$options = ['(any)', 'confirmed', 'pending'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('st') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select></td></tr>
<tr><td valign="middle" class=rowhead>Email:</td>
  <td<?php echo SupportContext::getQuery('em') ? $highlight : ''?>><input name="em" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('em'))?>" size="35"></td>
  <td valign="middle" class=rowhead>IP:</td>
  <td<?php echo SupportContext::getQuery('ip') ? $highlight : ''?>><input name="ip" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('ip'))?>" maxlength="64"></td>

  <td valign="middle" class=rowhead>Account status:</td>
  <td<?php echo SupportContext::getQuery('as') ? $highlight : ''?>><select name="as">
<?php
$options = ['(any)', 'enabled', 'disabled'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('as') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select></td></tr>
<tr>
  <td valign="middle" class=rowhead>Comment:</td>
  <td<?php echo SupportContext::getQuery('co') ? $highlight : ''?>><input name="co" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('co'))?>" size="35"></td>
  <td valign="middle" class=rowhead>Mask:</td>
  <td<?php echo SupportContext::getQuery('ma') ? $highlight : ''?>><input name="ma" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('ma'))?>" maxlength="17"></td>
  <td valign="middle" class=rowhead>Class:</td>
  <td<?php echo (SupportContext::getQuery('c') && SupportContext::getQuery('c') != 1) ? $highlight : ''?>><select name="c"><option value='1'>(any)</option>
<?php
  $class = SupportContext::getQuery('c');
...
<tr>
  ...
  <td<?php echo SupportContext::getQuery('d') ? $highlight : ''?>><select name="dt">
<?php
    $options = ['on', 'before', 'after', 'between'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('dt') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select>

    <input name="d" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('d'))?>" size="12" maxlength="10">

    <input name="d2" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('d2'))?>" size="12" maxlength="10"></td>


  <td valign="middle" class=rowhead>Uploaded:</td>

  <td<?php echo SupportContext::getQuery('ul') ? $highlight : ''?>><select name="ult" id="ult">
<?php
    $options = ['equal', 'above', 'below', 'between'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('ult') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select>

    <input name="ul" type="text" id="ul" size="8" maxlength="7" value="<?php echo htmlspecialchars(SupportContext::getQuery('ul'))?>">

    <input name="ul2" type="text" id="ul2" size="8" maxlength="7" value="<?php echo htmlspecialchars(SupportContext::getQuery('ul2'))?>"></td>
  <td valign="middle" class="rowhead">Donor:</td>

  <td<?php echo SupportContext::getQuery('do') ? $highlight : ''?>><select name="do">
<?php
    $options = ['(any)', 'Yes', 'No'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('do') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
	</select></td></tr>
<tr>
...
<td valign="middle" class=rowhead>Last seen:</td>
  ...
  <td <?php echo SupportContext::getQuery('ls') ? $highlight : ''?>><select name="lst">
<?php
  $options = ['on', 'before', 'after', 'between'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('lst') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
  </select>

  <input name="ls" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('ls'))?>" size="12" maxlength="10">

  <input name="ls2" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('ls2'))?>" size="12" maxlength="10"></td>
	  <td valign="middle" class=rowhead>Downloaded:</td>

  <td<?php echo SupportContext::getQuery('dl') ? $highlight : ''?>><select name="dlt" id="dlt">
<?php
    $options = ['equal', 'above', 'below', 'between'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('dlt') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
    </select>

    <input name="dl" type="text" id="dl" size="8" maxlength="7" value="<?php echo htmlspecialchars(SupportContext::getQuery('dl'))?>">

    <input name="dl2" type="text" id="dl2" size="8" maxlength="7" value="<?php echo htmlspecialchars(SupportContext::getQuery('dl2'))?>"></td>

	<td valign="middle" class=rowhead>Warned:</td>

	<td<?php echo SupportContext::getQuery('w') ? $highlight : ''?>><select name="w">
<?php
  $options = ['(any)', 'Yes', 'No'];
for ($i = 0; $i < count($options); $i++) {
    echo "<option value=$i ".((SupportContext::getQuery('w') == "$i") ? 'selected' : '').'>'.$options[$i]."</option>\n";
}
?>
	</select></td></tr>
...
<tr><td class="rowhead"></td><td></td>
  <td valign="middle" class=rowhead>Active only:</td>
	<td<?php echo SupportContext::getQuery('ac') ? $highlight : ''?>><input name="ac" type="checkbox" value="1" <?php echo (SupportContext::getQuery('ac')) ? 'checked' : '' ?>></td>
  <td valign="middle" class=rowhead>Disabled IP: </td>
  <td<?php echo SupportContext::getQuery('dip') ? $highlight : ''?>><input name="dip" type="checkbox" value="1" <?php echo (SupportContext::getQuery('dip')) ? 'checked' : '' ?>></td>
  </tr>
...
// ratio as a string
if (! function_exists('ratios')) {
    function ratios(float $up, float $down, bool $color = true): string
    {
        if ($down > 0) {
...
if (count(SupportContext::allQuery()) > 0 && empty(SupportContext::getQuery('h'))) {
    try {
        $searchResult = UserSearchRepository::administrativeSearch((array) SupportContext::allQuery(), (bool) $hasModcomment, 30);
    } catch (InvalidArgumentException $e) {
        Html::stdMessage('Error', $e->getMessage());
    ...
    $q = $searchResult['q'];
    $perpage = 30;
    [$pagertop, $pagerbottom, , $offset, $rpp] = Pagination::pager($perpage, $count, $__server_REQUEST_URI.'?'.$q);
    $res = $searchResult['rows'];

    $userIds = array_map(fn ($row) => (int) ($row['id'] ?? 0), $res);
    $ips = array_map(fn ($row) => (string) ($row['ip'] ?? ''), $res);
    $extraStats = UserListingRepository::getSearchExtraStats($userIds, $ips, (int) $CURUSER['class']);
    ...
    if (count($res) == 0) {
        Html::stdMessage('Warning', 'No user was found.');
    } else {
        if ($count > $perpage) {
            echo $pagertop;
        }
        ...
        foreach ($res as $user) {
            ...
            echo '<tr><td>'.
                  UserDisplay::username($user['id']).'</td>'.
