...
final class UsercpTrackerSectionBuilder
{
    ...
    public function build(array $lang, array $curUser): array
    {
        ...
        $rowsHtml .= (string) Html::trSmall($lang['row_browse_default_categories'] ?? 'Browse categories', $categoriesCell, 1, '', true);

        $rowsHtml .= (string) Html::trSmall($lang['row_stylesheet'] ?? 'Stylesheet', "<select name=stylesheet>\n".$stylesheetOptions."\n</select>&nbsp;&nbsp;<font class=small>".htmlspecialchars($lang['text_stylesheet_note'] ?? '').'<a href="aboutnexus.php#stylesheet" ><b>'.htmlspecialchars($lang['text_stylesheet_link'] ?? '').'</b></a></font>.', 1, '', true);

        $rowsHtml .= (string) Html::trSmall($lang['row_font_size'] ?? 'Font size', '<select name=fontsize><option value=small '.(($curUser['fontsize'] ?? '') === 'small' ? ' selected' : '').'>'.htmlspecialchars($lang['select_small'] ?? '').'</option><option value=medium '.(($curUser['fontsize'] ?? '') === 'medium' ? ' selected' : '').'>'.htmlspecialchars($lang['select_medium'] ?? '').'</option><option value=large '.(($curUser['fontsize'] ?? '') === 'large' ? ' selected' : '').'>'.htmlspecialchars($lang['select_large'] ?? '').'</option></select>', 1, '', true);

        $rowsHtml .= (string) Html::trSmall($lang['row_site_language'] ?? 'Site language', "<select name=\"sitelanguage\">\n".$langOptions."\n</select>&nbsp;&nbsp;<font class=small>".htmlspecialchars($lang['text_translation_note'] ?? '').'<a href="aboutnexus.php#translation"><b>'.htmlspecialchars($lang['text_translation_link'] ?? '').'</b></a></font>.', 1, '', true);

        $rowsHtml .= (string) Html::trSmall($lang['row_pm_boxes'] ?? 'PM boxes', htmlspecialchars($lang['text_show'] ?? '').'<input type=text name=pmnum size=5 value='.(int) ($curUser['pmnum'] ?? 0).' >'.htmlspecialchars($lang['text_pms_per_page'] ?? ''), 1, '', true);

        if ((string) $this->globals->get('showshoutbox_main', '') === 'yes') {
            $rowsHtml .= (string) Html::trSmall($lang['row_shoutbox'] ?? 'Shoutbox', htmlspecialchars($lang['text_show_last'] ?? '').'<input type=text name=sbnum size=5 value='.(int) ($curUser['sbnum'] ?? 0).' >'.htmlspecialchars($lang['text_messages_at_shoutbox'] ?? '').'<br />'.htmlspecialchars($lang['text_refresh_shoutbox_every'] ?? '').'<input type=text name=sbrefresh size=5 value='.(int) ($curUser['sbrefresh'] ?? 0).' >'.htmlspecialchars($lang['text_seconds'] ?? ''), 1, '', true);
        }

        $rowsHtml .= (string) Html::trSmall($lang['row_torrent_detail'] ?? 'Torrent detail', '<input type=checkbox name=showdescription'.(LegacyYesNo::isYes($curUser['showdescription'] ?? null) ? ' checked' : '').' value=yes>'.htmlspecialchars($lang['text_show_description'] ?? ''), 1, '', true);
        $rowsHtml .= (string) Html::trSmall($lang['row_discuss'] ?? 'Comments', '<input type=checkbox name=showcomment'.(LegacyYesNo::isYes($curUser['showcomment'] ?? null) ? ' checked' : '').' value=yes>'.htmlspecialchars($lang['text_show_comments'] ?? ''), 1, '', true);
        $rowsHtml .= (string) Html::trSmall($lang['row_time_type'] ?? 'Time type', '<input type=radio name=timetype '.(($curUser['timetype'] ?? '') === 'timeadded' ? ' checked' : '').' value=timeadded>'.htmlspecialchars($lang['text_time_added'] ?? '').'&nbsp;&nbsp;<input type=radio name=timetype '.(($curUser['timetype'] ?? '') === 'timealive' ? ' checked' : '').' value=timealive>'.htmlspecialchars($lang['text_time_elapsed'] ?? '').'<br />', 1, '', true);
...
<b>'.htmlspecialchars($lang['text_comments_reviews'] ?? '').': </b><br /><input type=checkbox name=showcomnum '.(LegacyYesNo::isYes($curUser['showcomnum'] ?? null) ? ' checked' : '').' value=yes>'.($lang['text_show_comment_number'] ?? '').($showTooltipSetting ? '<select name="showlastcom" style="width: 70px;"><option value="yes" '.(! LegacyYesNo::isNo($curUser['showlastcom'] ?? null) ? ' selected' : '').'>'.htmlspecialchars($lang['select_with'] ?? '').'</option><option value="no" '.(LegacyYesNo::isNo($curUser['showlastcom'] ?? null) ? ' selected' : '').'>'.htmlspecialchars($lang['select_without'] ?? '').'</option></select>'.($lang['text_last_comment_on_tooltip'] ?? '') : '');
        $rowsHtml .= (string) Html::trSmall($lang['row_browse_page'] ?? 'Browse page', $browseCell, 1, '', true);
