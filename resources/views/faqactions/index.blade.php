...
@if (($mode ?? '') === 'edit')
    ...
    @elseif (($arr['type'] ?? '') === 'item')
        <form method="post" action="faqactions.php?action=edititem">
            ...
            <div class="nx-fcell">Question:</div><div class="nx-fcell"><input type="text" name="question" value="{{ $arr['question'] ?? '' }}" /></div>
            <div class="nx-fcell">Answer:</div><div class="nx-fcell"><textarea rows=20 name="answer">{{ $arr['answer'] ?? '' }}</textarea></div>
            <div class="nx-fcell">Status:</div><div class="nx-fcell">
                <select name="flag">
                    <option value="0"@if (($arr['flag'] ?? -1) == 0) selected="selected"@endif>Hidden</option>
                    <option value="1"@if (($arr['flag'] ?? -1) == 1) selected="selected"@endif>Normal</option>
                    <option value="2"@if (($arr['flag'] ?? -1) == 2) selected="selected"@endif>Updated</option>
                    <option value="3"@if (($arr['flag'] ?? -1) == 3) selected="selected"@endif>New</option>
                </select>
            </div>
    ...
    @elseif (($arr['type'] ?? '') === 'categ')
        <form method="post" action="faqactions.php?action=editsect">
            ...
            <div class="nx-fcell">Title:</div><div class="nx-fcell"><input type="text" name="title" value="{{ $arr['question'] ?? '' }}" /></div>
            <div class="nx-fcell">Status:</div><div class="nx-fcell">
                <select name="flag">
                    <option value="0"@if (($arr['flag'] ?? -1) == 0) selected="selected"@endif>Hidden</option>
                    <option value="1"@if (($arr['flag'] ?? -1) == 1) selected="selected"@endif>Normal</option>
...
@elseif (($mode ?? '') === 'additem')
    ...
    <form method="post" action="faqactions.php?action=addnewitem">
        ...
        <div class="nx-fcell">Answer:</div><div class="nx-fcell"><textarea rows=20 name="answer"></textarea></div>
        <div class="nx-fcell">Status:</div><div class="nx-fcell">
            <select name="flag">
