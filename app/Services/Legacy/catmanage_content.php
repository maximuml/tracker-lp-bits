...
if (UserDisplay::currentClass() < UC_ADMINISTRATOR) {
    LegacyResponse::permissionDenied();
}

$perpage = 50;
$pagerParam = '?action=view&type='.(SupportContext::getQuery('type') ?? 'searchbox').'&';
$lang_catmanage = (array) (SupportContext::getGlobal('lang_catmanage') ?? []);
SupportContext::setGlobal('perpage', $perpage);
SupportContext::setGlobal('pagerParam', $pagerParam);
if (! function_exists('return_category_db_table_name')) {
...
if (! function_exists('return_category_mode_selection')) {
    function return_category_mode_selection($selname, $selectedid)
    {
        $rows = CategoryRepository::getSearchboxOptions();
...
if (! function_exists('category_icon_selection')) {
    function category_icon_selection($iconId = 0)
    {
        $rows = CategoryRepository::getCaticonOptions();
...
if (! function_exists('return_type_name')) {
...
if (! function_exists('print_type_list')) {
...
if (! function_exists('check_valid_type')) {
    function check_valid_type($type)
    {
        $lang_catmanage = (array) (SupportContext::getGlobal('lang_catmanage') ?? []);
        $validtype = ['searchbox', 'caticon', 'secondicon', 'category', 'source', 'medium', 'codec', 'standard', 'processing', 'audiocodec'];
        if (! in_array($type, $validtype)) {
            LegacyResponse::abort($lang_catmanage['std_error'], $lang_catmanage['std_invalid_type']);
        }
    }
}
if (! function_exists('print_sub_category_list')) {
    function print_sub_category_list($type)
    {
        $lang_catmanage = (array) (SupportContext::getGlobal('lang_catmanage') ?? []);
        $perpage = SupportContext::getGlobal('perpage');
        $pagerParam = SupportContext::getGlobal('pagerParam');
        $dbtablename = return_category_db_table_name($type);
        $num = CategoryRepository::countByTable($dbtablename);
        if (! $num) {
            echo '<p align="center">'.$lang_catmanage['text_no_record_yet'].'</p>';
        } else {
            [$pagertop, $pagerbottom, , $offset, $perpage] = Pagination::pager($perpage, $num, $pagerParam);
            $rows = CategoryRepository::listByTable($dbtablename, (int) $offset, (int) $perpage, 'id', 'desc');
            ?>
...
if (! function_exists('print_category_editor')) {
    function print_category_editor($type, $row = '')
    {
        $lang_catmanage = (array) (SupportContext::getGlobal('lang_catmanage') ?? []);
        $validsubcattype = SupportContext::getGlobal('validsubcattype');
...
<?php
                    if ($type == 'searchbox') {
                        ...
                        Html::tr($lang_catmanage['row_searchbox_name'].'<font color="red">*</font>', '<input type="text" name="name" value="'.htmlspecialchars($name).'" style="width: 300px" /> '.$lang_catmanage['text_searchbox_name_note'], 1);
                        Html::tr($lang_catmanage['row_show_sub_category'], '<input type="checkbox" name="showsource" value="1"'.($showsource ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_sources'].'<input type="checkbox" name="showmedium" value="1"'.($showmedium ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_media'].'<input type="checkbox" name="showcodec" value="1"'.($showcodec ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_codecs'].'<input type="checkbox" name="showstandard" value="1"'.($showstandard ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_standards'].'<input type="checkbox" name="showprocessing" value="1"'.($showprocessing ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_processings'].'<input type="checkbox" name="showaudiocodec" value="1"'.($showaudiocodec ? ' checked="checked"' : '').' /> '.$lang_catmanage['text_audio_codecs'].'<br />'.$lang_catmanage['text_show_sub_category_note'], 1);

                        // extra
                        $extraCheckbox = '';
                        foreach (SearchBox::listExtraText() as $name => $text) {
                            $extraCheckbox .= sprintf(
                                '<label><input type="checkbox" name="extra[%s]" value="1"%s />%s</label>',
                                $name, ! empty($row['extra'][$name]) ? ' checked' : '', $text
                            );
                        }
                        Html::tr($lang_catmanage['row_searchbox_extras'], $extraCheckbox, 1);

                        Html::tr($lang_catmanage['row_items_per_row'].'<font color="red">*</font>', '<input type="text" name="catsperrow" value="'.$catsperrow.'" style="width: 100px" /> '.$lang_catmanage['text_items_per_row_note'], 1);
                        Html::tr($lang_catmanage['row_padding_between_items'].'<font color="red">*</font>', '<input type="text" name="catpadding" value="'.$catpadding.'" style="width: 100px" /> '.$lang_catmanage['text_padding_between_items_note'], 1);
                        $field = new Field;
                        Html::tr($lang_catmanage['row_enable_custom_field'], $field->buildFieldCheckbox('custom_fields[]', $row['custom_fields'] ?? ''), 1);
                        Html::tr($lang_catmanage['row_custom_field_display_name'], '<input type="text" name="custom_fields_display_name" style="width: 300px" value="'.($row['custom_fields_display_name'] ?? '').'" />', 1);
                        $helpText = '<br/>'.$lang_catmanage['row_custom_field_display_help'];
                        Html::tr($lang_catmanage['row_custom_field_display'], '<textarea name="custom_fields_display" style="width: 300px" rows="8">'.($row['custom_fields_display'] ?? '').'</textarea>'.$helpText, 1);
                    } elseif ($type == 'caticon') {
                        if ($row) {
                            $name = $row['name'];
                            $folder = $row['folder'];
...
<?php
                                    Html::tr($lang_catmanage['col_name'].'<font color="red">*</font>', '<input type="text" name="name" value="'.htmlspecialchars($name).'" style="width: 300px" /> ', 1);
                        Html::tr($lang_catmanage['col_folder'].'<font color="red">*</font>', '<input type="text" name="folder" value="'.htmlspecialchars($folder).'" style="width: 300px" /><br />'.$lang_catmanage['text_folder_note'], 1);
                        Html::tr($lang_catmanage['text_multi_language'], '<input type="checkbox" name="multilang" value="yes"'.($multilang == 'yes' ? ' checked="checked"' : '').' />'.$lang_catmanage['text_yes'].'<br />'.$lang_catmanage['text_multi_language_note'], 1);
                        Html::tr($lang_catmanage['text_second_icon'], '<input type="checkbox" name="secondicon" value="yes"'.($secondicon == 'yes' ? ' checked="checked"' : '').' />'.$lang_catmanage['text_yes'].'<br />'.$lang_catmanage['text_second_icon_note'], 1);
                        Html::tr($lang_catmanage['text_css_file'], '<input type="text" name="cssfile" value="'.htmlspecialchars($cssfile).'" style="width: 300px" /> '.$lang_catmanage['text_css_file_note'], 1);
                        Html::tr($lang_catmanage['text_designer'], '<input type="text" name="designer" value="'.htmlspecialchars($designer).'" style="width: 300px" /> '.$lang_catmanage['text_designer_note'], 1);
                        Html::tr($lang_catmanage['text_comment'], '<input type="text" name="comment" value="'.htmlspecialchars($comment).'" style="width: 300px" /> '.$lang_catmanage['text_comment_note'], 1);
                        ...
                        Html::tr($lang_catmanage['col_name'].'<font color="red">*</font>', '<input type="text" name="name" value="'.htmlspecialchars($name).'" style="width: 300px" /> '.$lang_catmanage['text_second_icon_name_note'], 1);
                        Html::tr($lang_catmanage['col_image'].'<font color="red">*</font>', '<input type="text" name="image" value="'.htmlspecialchars($image).'" style="width: 300px" /><br />'.$lang_catmanage['text_image_note'], 1);
                        Html::tr($lang_catmanage['text_class_name'], '<input type="text" name="class_name" value="'.htmlspecialchars($class_name).'" style="width: 300px" /><br />'.$lang_catmanage['text_class_name_note'], 1);
                        Html::tr($lang_catmanage['row_selections'].'<font color="red">*</font>', Html::torrentSelection(return_type_name('source'), 'source', return_category_db_table_name('source'), $source).Html::torrentSelection(return_type_name('medium'), 'medium', return_category_db_table_name('medium'), $medium).Html::torrentSelection(return_type_name('codec'), 'codec', return_category_db_table_name('codec'), $codec).Html::torrentSelection(return_type_name('standard'), 'standard', return_category_db_table_name('standard'), $standard).Html::torrentSelection(return_type_name('processing'), 'processing', return_category_db_table_name('processing'), $processing).Html::torrentSelection(return_type_name('audiocodec'), 'audiocodec', return_category_db_table_name('audiocodec'), $audiocodec).'<br />'.$lang_catmanage['text_selections_note'], 1);
                        ...
                        Html::tr($lang_catmanage['row_category_name'].'<font color="red">*</font>', '<input type="text" name="name" value="'.htmlspecialchars($name).'" style="width: 300px" /> '.$lang_catmanage['text_category_name_note'], 1);
                        Html::tr($lang_catmanage['col_image'].'<font color="red">*</font>', '<input type="text" name="image" value="'.htmlspecialchars($image).'" style="width: 300px" /><br />'.$lang_catmanage['text_image_note'], 1);
                        Html::tr($lang_catmanage['text_class_name'], '<input type="text" name="class_name" value="'.htmlspecialchars($class_name).'" style="width: 300px" /><br />'.$lang_catmanage['text_class_name_note'], 1);
                        Html::tr($lang_catmanage['row_mode'].'<font color="red">*</font>', return_category_mode_selection('mode', $mode), 1);
                        Html::tr($lang_catmanage['text_category_icons'].'<font color="red">*</font>', category_icon_selection($row['icon_id'] ?? 0), 1);
                        Html::tr($lang_catmanage['col_order'], '<input type="text" name="sort_index" value="'.$sort_index.'" style="width: 100px" /> '.$lang_catmanage['text_order_note'], 1);
                    }
            ?>
</table>
</div>
...
if (! function_exists('print_sub_category_editor')) {
...
<table border="1" cellspacing="0" cellpadding="10" width="100%">
<?php
        Html::tr($lang_catmanage['col_name'].'<font color="red">*</font>', '<input type="text" name="name" value="'.htmlspecialchars($name).'" style="width: 300px" /> '.$lang_catmanage['text_subcategory_name_note'], 1);
        Html::tr($lang_catmanage['col_order'], '<input type="text" name="sort_index" value="'.$sort_index.'" style="width: 100px" /> '.$lang_catmanage['text_order_note'], 1);
        ?>
</table>
<div style="text-align: center; margin-top: 10px;">
...
SupportContext::setGlobal('validsubcattype', $validsubcattype);
$type = SupportContext::getQuery('type') ?? '';
if ($type == '') {
    $type = 'searchbox';
} else {
    check_valid_type($type);
}
$action = SupportContext::getQuery('action') ?? '';
if ($action == '') {
    $action = 'view';
}
if ($action == 'view') {
...
<?php
        if (in_array($type, $validsubcattype)) {
            ...
            if (! $num) {
                ...
                [$pagertop, $pagerbottom, , $offset, $perpage] = Pagination::pager($perpage, $num, $pagerParam);
                $rows = CategoryRepository::listByTable($dbtablename, (int) $offset, (int) $perpage, 'id', 'asc');
                ?>
<table border="1" cellspacing="0" cellpadding="5" width="97%">
<tr>
<td class="colhead"><?php echo $lang_catmanage['col_id']?></td>
...
<?php
            ...
            $lookups = CategoryRepository::getSecondiconLookups();
...
<?php
            ...
            if (! $num) {
                ...
                [$pagertop, $pagerbottom, , $offset, $perpage] = Pagination::pager($perpage, $num, $pagerParam);
                $rows = CategoryRepository::getCategoryList((int) $offset, (int) $perpage);
...
<?php
    ...
    if (! $id) {
        LegacyResponse::abort($lang_catmanage['std_error'], $lang_catmanage['std_invalid_id']);
    } else {
        $dbtablename = return_category_db_table_name($type);
        $row = CategoryRepository::getRecord($dbtablename, $id);
        if (! $row) {
            LegacyResponse::abort($lang_catmanage['std_error'], $lang_catmanage['std_invalid_id']);
        } else {
            $typename = return_type_name($type);
            echo '<form method="post" action="?action=submit&amp;type='.$type.'">';
    ...
    echo 'This method is deprecated! This method is no longer available in 1.8, it does not save data correctly, please go to the management system!';
