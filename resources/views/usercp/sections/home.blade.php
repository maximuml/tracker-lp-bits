...
@php
// Seed box
if ($home['seedBox']['enabled']):
    $sb = $home['seedBox'];
    $seedBoxHtml = '';
    if (count($sb['records']) > 0):
        $seedBoxHtml .= "<table border='1' cellspacing='0' cellpadding='5' id='seed-box-table'><tr><td class='colhead'>ID</td><td class='colhead'>{$sb['columns']['operator']}</td><td class='colhead'>{$sb['columns']['bandwidth']}</td><td class='colhead'>{$sb['columns']['ip']}</td><td class='colhead'>{$sb['columns']['comment']}</td><td class='colhead'>{$sb['columns']['status']}</td><td class='colhead'></td></tr>";
        foreach ($sb['records'] as $record):
            $ipCell = ! empty($record->ip) ? htmlspecialchars((string) $record->ip) : sprintf('%s ~ %s', htmlspecialchars((string) $record->ip_begin), htmlspecialchars((string) $record->ip_end));
            $seedBoxHtml .= '<tr>';
            $seedBoxHtml .= sprintf('<td>%s</td>', (int) $record->id);
            $seedBoxHtml .= sprintf('<td>%s</td>', htmlspecialchars((string) $record->operator));
            $seedBoxHtml .= sprintf('<td>%s</td>', htmlspecialchars((string) ($record->bandwidth ?: '')));
            $seedBoxHtml .= sprintf('<td>%s</td>', $ipCell);
            $seedBoxHtml .= sprintf('<td>%s</td>', htmlspecialchars((string) $record->comment));
            $seedBoxHtml .= sprintf('<td>%s</td>', htmlspecialchars((string) $record->statusText));
            $seedBoxHtml .= sprintf('<td><img style="cursor: pointer" class="staff_delete remove-seed-box-btn" src="pic/trans.gif" alt="D" title="%s" data-id="%s"></td>', htmlspecialchars($sb['deleteLabel']), (int) $record->id);
            $seedBoxHtml .= '</tr>';
        endforeach;
        $seedBoxHtml .= '</table>';
    endif;
    $seedBoxHtml .= sprintf('<div><input type="button" id="add-seed-box-btn" value="%s"/></div>', htmlspecialchars($sb['addLabel']));
    \App\Support\Html::trSmall($sb['rowLabel'], $seedBoxHtml, 1);

    $seedBoxForm = <<<FORM
<div class="form-box">
<form id="seed-box-form">
    <div class="form-control-row">
        <div class="label">{$sb['columns']['operator']}</div>
        <div class="field"><input type="text" name="params[operator]"></div>
    </div>
    <div class="form-control-row">
        <div class="label">{$sb['columns']['bandwidth']}</div>
        <div class="field"><input type="number" name="params[bandwidth]"></div>
    </div>
    <div class="form-control-row">
        <div class="label">{$sb['columns']['ip']}</div>
        <div class="field"><input type="text" name="params[ip]"></div>
    </div>
    <div class="form-control-row">
        <div class="label">{$sb['columns']['comment']}</div>
        <div class="field"><textarea name="params[comment]" rows="4"></textarea></div>
    </div>
</form>
</div>
FORM;
    $confirmRemove = addslashes($sb['confirmRemoveLabel']);
    $rowLabel = addslashes($sb['rowLabel']);
    $addLabel = addslashes($sb['addLabel']);
    $deleteLabel = addslashes($sb['deleteLabel']);
    $seedBoxJs = <<<JS
jQuery('#add-seed-box-btn').on('click', function () {
    layer.open({
        type: 1,
        title: "{$rowLabel} {$addLabel}",
        content: `{$seedBoxForm}`,
        btn: ['OK'],
        btnAlign: 'c',
        yes: function () {
...
jQuery('#seed-box-table').on('click', '.remove-seed-box-btn', function () {
    ...
    layer.confirm("{$confirmRemove}", window.nexusLayerOptions.confirm, function (index) {
        jQuery('body').loading({stoppable: false});
