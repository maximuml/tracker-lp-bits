...
@if ($action === 'viewposts')
    ...
    @foreach ($items ?? [] as $item)
        ...
        &nbsp;--&nbsp;@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(__('legacy/userhistory.text_post')))
...
@elseif ($action === 'viewcomments')
    ...
    @foreach ($items ?? [] as $item)
        ...
        @else
            [Deleted]
        @endif
        &nbsp;---&nbsp;@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(__('legacy/userhistory.text_comment')))</b>#<a href=details.php?id={{ $item['torrentid'] }}&tocomm=1&hit=1{{ $item['pageUrl'] }}>{{ $item['commentid'] }}</a>
        </p>
        <br />
        <table data-nx="data" class=main width=100% border=1 cellspacing=0 cellpadding=5>
        <tr valign=top><td class=comment>{{ $item['bodyHtml'] }}</td></tr>
