...
final class MeiliSearchRepositoryQueryTest extends TestCase
{
    ...
    public function test_get_query_handles_array_search_input(): void
    {
        ...
        $method = new \ReflectionMethod($service, 'getQuery');
        $method->setAccessible(true);

        $result = $method->invoke($service, ['search' => ['x']]);
    ...
    public function test_get_query_trims_scalar_search_in_and_mode(): void
    {
        ...
        $result = $method->invoke($service, ['search' => '  foo bar  ', 'search_mode' => SearchBox::SEARCH_MODE_AND]);
    ...
    public function test_get_query_wraps_scalar_search_in_exact_mode(): void
    {
        ...
        $result = $method->invoke($service, ['search' => '  foo bar  ', 'search_mode' => SearchBox::SEARCH_MODE_EXACT]);
    ...
    {
        $service = app(MeiliSearchService::class);
        $method = new \ReflectionMethod($service, 'getSearchArea');
        $method->setAccessible(true);

        $result = $method->invoke($service, ['search_area' => ['x']]);
    ...
    public function test_get_search_area_returns_valid_scalar_value(): void
    {
        ...
        $result = $method->invoke($service, ['search_area' => MeiliSearchRepository::SEARCH_AREA_OWNER]);
