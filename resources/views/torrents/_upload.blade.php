	<form id="compose" enctype="multipart/form-data" action="/takeupload" method="post" name="upload">
			...
			<div class="nx-fgrid">
				...
				<x-settings-row layout="grid" :label="(__('legacy/upload.row_torrent_file')).'<font color=red>*</font>'">
...
<b>{{ __('legacy/upload.text_chinese_title') ?? '' }}</b>&nbsp;<input type="text" style="width: 250px" name="cnname"><br /><font class="medium">{{ __('legacy/upload.text_titles_note') ?? '' }}</font>
				...
				<x-settings-row layout="grid" :label="(__('legacy/upload.row_type')).'<font color=red>*</font>'">
				...
				@if (! empty($offerRows))
					<x-settings-row layout="grid" :label="(__('legacy/upload.row_your_offer')).(!$uploadFreely ? '<font color=red>*</font>' : '')">
						<select name="offer">
							<option value="0">{{ __('legacy/upload.select_choose_one') ?? '' }}</option>
