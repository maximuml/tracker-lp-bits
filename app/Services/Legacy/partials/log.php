...
if (! isset($lang_log)) {
    $lang_log = (array) (SupportContext::getGlobal('lang_log') ?? []);
}
...
if ($mode === 'dailylog') {
    ...
    <?php
    if (empty($logRows)) {
        echo $lang_log['text_log_empty'] ?? 'Log is empty.';
    } else {
        echo "<table width=940 border=1 cellspacing=0 cellpadding=5>\n";
        echo '<tr><td class=colhead align=center><img class="time" src="pic/trans.gif" alt="time" title="'.($lang_log['title_time_added'] ?? 'Time added').'" /></td><td class=colhead align=left>'.($lang_log['col_event'] ?? 'Event').'';
        if ($canConfidentialLog) {
            echo '<td class=colhead align=left>'.($lang_log['col_user'] ?? 'User').'</td>';
        }
        echo "</td></tr>\n";
        foreach ($logRows as $arr) {
            $color = '';
            $txt = (string) ($arr['txt'] ?? '');
            if (strpos($txt, 'was uploaded by') !== false) {
                $color = 'green';
            }
            if (strpos($txt, 'was deleted by') !== false) {
                $color = 'red';
            }
            if (strpos($txt, 'was added to the Request section') !== false) {
                $color = 'purple';
            }
            if (strpos($txt, 'was edited by') !== false) {
                $color = 'blue';
            }
            if (strpos($txt, 'settings updated by') !== false) {
                $color = 'darkred';
            }
            echo '<tr><td class="rowfollow nowrap" align=center>'.Time::format((string) ($arr['added'] ?? ''), true, false)."</td><td class=rowfollow align=left><font color='".$color."'>".htmlspecialchars($txt).'</font></td>';
            if ($canConfidentialLog) {
                $uid = (int) ($arr['uid'] ?? 0);
                echo '<td class=rowfollow align=left>'.($uid > 0 ? ($userDisplayMap[$uid] ?? UserDisplay::username($uid)) : 'System').'</td>';
            }
            echo "</tr>\n";
        }
        echo '</table>';
    ...
    if (empty($chronicleRows)) {
        echo $lang_log['text_chronicle_empty'] ?? 'Chronicle is empty.';
    } else {
        echo "<table width=940 border=1 cellspacing=0 cellpadding=5>\n";
        echo '<tr><td class=colhead align=center>'.($lang_log['col_date'] ?? 'Date').'</td><td class=colhead align=left>'.($lang_log['col_event'] ?? 'Event').'</td>'.($canManage ? '<td class=colhead align=center>'.($lang_log['col_modify'] ?? 'Modify').'</td>' : '')."</tr>\n";
        foreach ($chronicleRows as $arr) {
            $date = Time::format((string) ($arr['added'] ?? ''), true, false);
            echo "<tr><td class=rowfollow align=center><nobr>$date</nobr></td><td class=rowfollow align=left>".Format::formatComment((string) ($arr['txt'] ?? ''), true, false, true).'</td>'.($canManage ? '<td align=center nowrap><b><a href="?action=chronicle&do=edit&id='.(int) ($arr['id'] ?? 0).'">'.($lang_log['text_edit'] ?? 'Edit').'</a>&nbsp;|&nbsp;<a href="?action=chronicle&do=del&id='.(int) ($arr['id'] ?? 0).'"><font color=red>'.($lang_log['text_delete'] ?? 'Delete').'</font></a></b></td>' : '')."</tr>\n";
        }
        echo '</table>';
        echo $pagerbottom;
    }
