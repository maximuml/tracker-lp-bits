...
if (!isset($lang_complains)) $lang_complains = (array) (\App\Support\SupportContext::getGlobal('lang_complains') ?? []);
...
if ($mode === 'list') {
    ...
    if (($page ?? null) === null) {
        ...
        if (! empty($pendingRows)) {
            ...
            echo \App\Support\Html::tableRow('colhead', $lang_complains['th_complain_at'] ?? 'Added', $lang_complains['th_complain_account'] ?? 'Account', $lang_complains['th_action_view'] ?? 'View');
            foreach ($pendingRows as $row) {
                $row = (array) $row;
                echo \App\Support\Html::tableRow('rowfollow', \App\Support\Time::format($row['added'] ?? ''), htmlspecialchars((string) ($row['email'] ?? '')), sprintf('<a href="?action=view&id=%s" class="faqlink">%s</a>', htmlspecialchars((string) ($row['uuid'] ?? '')), $lang_complains['th_action_view'] ?? 'View'));
            }
            echo '</table>';
        } else {
    ...
    printf('%s：%s<br />%s %s', $lang_complains['text_added'] ?? 'Added', \App\Support\Time::format($complain['added'] ?? ''), $lang_complains['text_new_email'] ?? 'Email', htmlspecialchars((string) ($complain['email'] ?? '')));
    if ($isAdmin) {
        if (! empty($user)) {
    ...
    if (! empty($replyRows)) {
        foreach ($replyRows as $r) {
            $row = (array) $r;
            printf('<b>%s @ %s', (int) ($row['userid'] ?? 0) ? ($replyUserMap[(int) ($row['userid'] ?? 0)] ?? \App\Support\UserDisplay::plainUsername((int) ($row['userid'] ?? 0))) : ($lang_complains['text_complainer'] ?? 'Complainer'), \App\Support\Time::format($row['added'] ?? ''));
            if ($isAdmin) {
    ...
    if (! empty($complain['answered']) && (int) $complain['answered'] !== 0) {
        ...
        \App\Support\Html::quickReplyVoid('reply', 'body', $lang_complains['text_reply'] ?? 'Reply');
