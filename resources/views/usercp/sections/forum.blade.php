...
\App\Support\Html::trSmall($lang_usercp['row_view_avatars'] ?? 'View avatars', '<input type=checkbox name=avatars'.(\App\Support\LegacyYesNo::isYes($CURUSER['avatars'] ?? null) ? ' checked' : '').'>'.htmlspecialchars($lang_usercp['checkbox_low_bandwidth_note'] ?? ''), 1);
\App\Support\Html::trSmall($lang_usercp['row_view_signatures'] ?? 'View signatures', '<input type=checkbox name=signatures'.(\App\Support\LegacyYesNo::isYes($CURUSER['signatures'] ?? null) ? ' checked' : '').'>'.htmlspecialchars($lang_usercp['checkbox_low_bandwidth_note'] ?? ''), 1);
if ($showtooltipsetting):
    \App\Support\Html::tr($lang_usercp['row_tooltip_last_post'] ?? 'Tooltip last post', '<input type=checkbox name=ttlastpost'.(\App\Support\LegacyYesNo::isYes($CURUSER['showlastpost'] ?? null) ? ' checked' : '').'>'.htmlspecialchars($lang_usercp['checkbox_last_post_note'] ?? ''), 1);
endif;
