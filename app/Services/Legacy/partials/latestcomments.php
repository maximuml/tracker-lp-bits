...
if ($count == 0) {
    Html::stdMessage($lang_functions['text_sorry'] ?? 'Sorry', $lang_functions['text_no_comments'] ?? 'No comments yet.');
} else {
    echo $pagertop;
    ?>
            ...
            <table id="cid<?php echo $commentId; ?>" border="0" cellspacing="0" cellpadding="0" width="100%">
                <tr>
                    <td class="embedded" width="99%">
                        ...
                        <?php echo $userDisplayMap[$userId] ?? UserDisplay::username($userId, false, true, true, false, false, true); ?>
                        &nbsp;&nbsp;<font color="gray"><?php echo $lang_functions['text_at'] ?? 'at'; ?></font>
                        <?php echo Time::format($row['added'] ?? ''); ?>
                        <?php echo $parentLink; ?>
                    </td>
                </tr>
            </table>
            <table class="main" width="100%" border="0" cellspacing="0" cellpadding="5">
                <tr>
                    <td class="rowfollow" width="150" valign="top" style="padding: 0px;">
                        <?php echo UserDisplay::avatarImageWithContext($avatar); ?>
                    </td>
                    <td class="rowfollow word-break-all" valign="top">
                        <br />
                        <?php echo Format::formatComment((string) ($row['text'] ?? '')); ?>
                    </td>
                </tr>
            </table>
        </div>
