...
<?php
    ...
    if (empty($forums)) {
        ...
        foreach ($forums as $row) {
            ...
            echo '<td>' . htmlspecialchars((string) ($row['of_name'] ?? '')) . '</td><td>' . \App\Support\UserClass::name((int) $row['minclassread'], false, true, true) . '</td><td>' . \App\Support\UserClass::name((int) $row['minclasswrite'], false, true, true) . '</td><td>' . \App\Support\UserClass::name((int) $row['minclasscreate'], false, true, true) . '</td><td>' . $moderators . '</td><td><b><a href="forummanage.php?action=editforum&id=' . (int) $row['id'] . '">' . ($lang_forummanage['text_edit'] ?? 'Edit') . '</a>&nbsp;|&nbsp;<a href="#" data-confirm-del="' . (int) $row['id'] . '" data-confirm-note="' . ($lang_forummanage['js_sure_to_delete_forum'] ?? '') . '"><font color=red>' . ($lang_forummanage['text_delete'] ?? 'Delete') . '</font></a></b></td></tr>';
        }
    }
    echo '</table>';
endif;
