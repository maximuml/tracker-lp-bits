...
class SearchRepository extends BaseRepository
{
    ...
    {
        $elasticsearchEnabled = Env::get('ELASTICSEARCH_ENABLED', null);
    ...
    {
        if (is_null($this->es)) {
            $config = Config::get('nexus.elasticsearch', null);
    ...
    public function import($torrentId = null)
    {
        ...
        while (true) {
            ...
            if ($torrentResults->isEmpty()) {
                Logger::writeWithContext((string) "{$log}, no more data...", (string) 'info', (bool) true);
                break;
            }
            Logger::writeWithContext((string) ("{$log}, get counts: ".$torrentResults->count()), (string) 'info', (bool) true);
    ...
    private function buildTorrentBody($torrent, bool $underlinePrefix = false): array
    {
        ...
        if ($searchBoxId == 0) {
            Logger::writeWithContext((string) sprintf('[INVALID_CATEGORY], Torrent: %s', $torrent->id), (string) 'error', (bool) false);
        }
    ...
    private function logEsResponse($msg, $response)
    {
        ...
        Logger::writeWithContext((string) $msg, (string) 'info', (bool) Environment::isConsole());
    }
    ...
    {
        if ($esResponse instanceof Elasticsearch) {
    ...
    public function buildQuery(array $params, $user, string $queryString)
    {
        ...
        foreach (self::$queryFieldToTorrentFieldMaps as $queryField => $torrentField) {
            if (isset($params[$queryField]) && $params[$queryField] !== '') {
                ...
                Logger::writeWithContext((string) "get mustBoolShould for {$torrentField} from params through {$queryField}: {$params[$queryField]}", (string) 'info', (bool) false);
            } elseif (preg_match_all("/{$queryField}([\d]+)=/", $queryString, $matches)) {
                if (count($matches) == 2 && ! empty($matches[1])) {
                    foreach ($matches[1] as $match) {
                        $mustBoolShould[$torrentField][] = ['match' => [$torrentField => $match]];
                        Logger::writeWithContext((string) "get mustBoolShould for {$torrentField} from params through {$queryField}: {$match}", (string) 'info', (bool) false);
                    }
                }
            } else {
                // get user setting
                ...
                if (preg_match($pattern, (string) $userSetting, $matches)) {
                    if (count($matches) == 2 && ! empty($matches[1])) {
                        $match = $matches[1];
                        $mustBoolShould[$torrentField][] = ['match' => [$torrentField => $match]];
                        Logger::writeWithContext((string) "get mustBoolShould for {$torrentField} from user setting through {$queryField}: {$match}", (string) 'info', (bool) false);
                    }
                }
            }
        }
        ...
        if (isset($params['incldead'])) {
            ...
            Logger::writeWithContext((string) 'maybe get must for visible from params', (string) 'info', (bool) false);
        } elseif (preg_match("/\[incldead=([\d]+)\]/", (string) $userSetting, $matches)) {
            $includeDead = (int) $matches[1];
            Logger::writeWithContext((string) 'maybe get must for visible from user setting', (string) 'info', (bool) false);
        }
        if ($includeDead == 1) {
            // active torrent
            $must[] = ['match' => ['visible' => 'yes']];
            Logger::writeWithContext((string) "get must for visible = yes through incldead: {$includeDead}", (string) 'info', (bool) false);
        } elseif ($includeDead == 2) {
            // dead torrent
            $must[] = ['match' => ['visible' => 'no']];
            Logger::writeWithContext((string) "get must for visible = no through incldead: {$includeDead}", (string) 'info', (bool) false);
        }

        $includeBookmarked = 0;
        if (isset($params['inclbookmarked'])) {
            $includeBookmarked = (int) $params['inclbookmarked'];
            Logger::writeWithContext((string) 'maybe get must or must_not for has_child.bookmark from params', (string) 'info', (bool) false);
        } elseif (preg_match("/\[inclbookmarked=([\d]+)\]/", (string) $userSetting, $matches)) {
            $includeBookmarked = (int) $matches[1];
            Logger::writeWithContext((string) 'maybe get must or must_not for has_child.bookmark from user setting', (string) 'info', (bool) false);
        }
        if ($includeBookmarked == 1) {
            // only bookmark
            $must[] = ['has_child' => ['type' => 'bookmark', 'query' => ['match' => ['user_id' => $user->id]]]];
            Logger::writeWithContext((string) "get must for has_child.bookmark through inclbookmarked: {$includeBookmarked}", (string) 'info', (bool) false);
        } elseif ($includeBookmarked == 2) {
            // only not bookmark
            $must_not[] = ['has_child' => ['type' => 'bookmark', 'query' => ['match' => ['user_id' => $user->id]]]];
            Logger::writeWithContext((string) "get must_not for has_child.bookmark through inclbookmarked: {$includeBookmarked}", (string) 'info', (bool) false);
        }

        $spState = 0;
        if (isset($params['spstate'])) {
            $spState = (int) $params['spstate'];
            Logger::writeWithContext((string) 'maybe get must for spstate from params', (string) 'info', (bool) false);
        } elseif (preg_match("/\[spstate=([\d]+)\]/", (string) $userSetting, $matches)) {
            $spState = (int) $matches[1];
            Logger::writeWithContext((string) 'maybe get must for spstate from user setting', (string) 'info', (bool) false);
        }
        if ($spState > 0) {
            $must[] = ['match' => ['sp_state' => $spState]];
            Logger::writeWithContext((string) "get must for sp_state = {$spState} through spstate: {$spState}", (string) 'info', (bool) false);
        }

        if (! empty($params['tag_id'])) {
            $must[] = ['has_child' => ['type' => 'tag', 'query' => ['match' => ['tag_id' => $params['tag_id']]]]];
            Logger::writeWithContext((string) "get must for has_child.tag through params.tag_id: {$params['tag_id']}", (string) 'info', (bool) false);
        }
            ...
            if ($searchMode == self::SEARCH_MODE_AND || $searchMode == self::SEARCH_MODE_EXACT) {
                ...
                if ($searchArea == self::SEARCH_AREA_TITLE) {
                    foreach ($keywordsArr as $keyword) {
                        ...
                        Logger::writeWithContext((string) "get must [SEARCH_MODE_AND + SEARCH_MODE_EXACT] for name match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                } elseif ($searchArea == self::SEARCH_AREA_DESC) {
                    foreach ($keywordsArr as $keyword) {
                        $must[] = ['match' => ["descr{$keywordFlag}" => $keyword]];
                        Logger::writeWithContext((string) "get must [SEARCH_MODE_AND + SEARCH_MODE_EXACT] for descr match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                } elseif ($searchArea == self::SEARCH_AREA_OWNER) {
                    foreach ($keywordsArr as $keyword) {
                        $must[] = ['has_parent' => ['parent_type' => 'user', 'query' => ['match' => ["username{$keywordFlag}" => $keyword]]]];
                        Logger::writeWithContext((string) "get must [SEARCH_MODE_AND + SEARCH_MODE_EXACT] has_parent.user match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                }
                ...
                if ($searchArea == self::SEARCH_AREA_TITLE) {
                    ...
                    foreach ($keywordsArr as $keyword) {
                        ...
                        Logger::writeWithContext((string) "get must bool should [SEARCH_MODE_OR] for name match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                    ...
                    foreach ($keywordsArr as $keyword) {
                        ...
                        Logger::writeWithContext((string) "get must bool should [SEARCH_MODE_OR] for descr match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                    $must[]['bool']['should'] = $tmpMustBoolShould;
                } elseif ($searchArea == self::SEARCH_AREA_OWNER) {
                    $tmpMustBoolShould = [];
                    foreach ($keywordsArr as $keyword) {
                        ...
                        Logger::writeWithContext((string) "get must bool should [SEARCH_MODE_OR] has_parent.user match '{$keyword}' through search", (string) 'info', (bool) false);
                    }
                    $must[]['bool']['should'] = $tmpMustBoolShould;
                }
            }
        ...
        Logger::writeWithContext((string) sprintf('params: %s, user: %s, queryString: %s, result: %s', Json::encode($params), $user->id, $queryString, Json::encode($result)), (string) 'info', (bool) false);
    ...
    public function listTorrentFromEs(array $params, $user, string $queryString)
    {
        ...
        if (! $response instanceof Elasticsearch) {
            Logger::writeWithContext((string) 'ES search response is not Elasticsearch', (string) 'error', (bool) false);
        ...
        if ($this->isEsResponseError($response)) {
            Logger::writeWithContext((string) ('error response: '.Json::encode($response)), (string) 'error', (bool) false);

            return $result;
        }
        if (empty($response['hits'])) {
            Logger::writeWithContext((string) ('empty response hits: '.Json::encode($response)), (string) 'error', (bool) false);

            return $result;
        }
        if ($response['hits']['total']['value'] == 0) {
            Logger::writeWithContext((string) ('total = 0, '.Json::encode($response)), (string) 'info', (bool) false);
    ...
    public function updateTorrent(int $id): bool
    {
        ...
        if ($this->isEsResponseError($result)) {
            Logger::writeWithContext((string) ("{$log}, fail: ".Json::encode($result)), (string) 'error', (bool) false);

            return false;
        }
        if (! is_array($result) || ! ($result['found'] ?? false)) {
            Logger::writeWithContext((string) "{$log}, not exists, do insert", (string) 'info', (bool) false);
        ...
        Logger::writeWithContext((string) ("{$log}, success: ".Json::encode($result)), (string) 'info', (bool) false);
    ...
    public function syncTorrentTags($torrent, $onlyDelete = false): bool
    {
        ...
        if ($this->isEsResponseError($result)) {
            Logger::writeWithContext((string) ("{$log}, delete torrent tag fail: ".Json::encode($result)), (string) 'error', (bool) false);

            return false;
        }
        Logger::writeWithContext((string) ("{$log}, delete torrent tag success: ".Json::encode($result)), (string) 'info', (bool) false);
        if ($onlyDelete) {
            Logger::writeWithContext((string) "{$log}, only delete, return true", (string) 'info', (bool) false);
        ...
        if (empty($bulk['body'])) {
            Logger::writeWithContext((string) "{$log}, no tags, return true", (string) 'info', (bool) false);

            return true;
        }
        $result = $this->getEs()->bulk($bulk);
        if ($this->isEsResponseError($result)) {
            Logger::writeWithContext((string) ("{$log}, insert torrent tag fail: ".Json::encode($result)), (string) 'error', (bool) false);

            return false;
        }
        Logger::writeWithContext((string) ("{$log}, insert torrent tag success: ".Json::encode($result)), (string) 'info', (bool) false);
