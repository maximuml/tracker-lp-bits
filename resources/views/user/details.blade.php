...
@endif
<x-settings-row-small :label="__('legacy/userdetails.row_join_date')">@if (($user['added'] ?? null) === null || $user['added'] === '0000-00-00 00:00:00'){{ __('legacy/userdetails.text_not_available') ?? '' }}@else{{ $user['added'] }} (@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(\App\Support\Time::format($user['added'], true, false, true))), {{ $joinWeeks }})@endif</x-settings-row-small>
<x-settings-row-small :label="__('legacy/userdetails.row_last_seen')">@if (($user['last_access'] ?? null) === null || $user['last_access'] === '0000-00-00 00:00:00'){{ __('legacy/userdetails.text_not_available') ?? '' }}@else{{ $user['last_access'] }} (@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(\App\Support\Time::format($user['last_access'], true, false, true))))@endif</x-settings-row-small>
@if (($where_tweak ?? '') === 'yes')
...
<x-settings-row-small :label="__('legacy/userdetails.row_gender')">@if (($user['gender'] ?? '') === 'Male')<img class='male' src='pic/trans.gif' alt='Male' title='{{ __('legacy/userdetails.title_male') ?? '' }}'="@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(__('legacy/userdetails.title_male')))" style='margin-left: 4pt' />@elseif (($user['gender'] ?? '') === 'Female')<img class='female' src='pic/trans.gif' alt='Female' title='{{ __('legacy/userdetails.title_female') ?? '' }}'="@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(__('legacy/userdetails.title_female')))" style='margin-left: 4pt' />@elseif (($user['gender'] ?? '') === 'N/A')<img class='no_gender' src='pic/trans.gif' alt='N/A' title='{{ __('legacy/userdetails.title_not_available') ?? '' }}'="@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(__('legacy/userdetails.title_not_available')))" style='margin-left: 4pt' />@endif</x-settings-row-small>
