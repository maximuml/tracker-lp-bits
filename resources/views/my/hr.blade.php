...
@if (! empty($rescount))
    ...
    <tr>
        ...
        <td class='rowfollow nowrap' align='center'>@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml(\App\Support\Ratio::hr($row->snatch->uploaded, $row->snatch->downloaded)))</td>
        ...
        <td class="rowfollow nowrap" align="center">
            @if ($row->uid == ($CURUSER['id'] ?? 0) && in_array($row->status, \App\Models\HitAndRun::CAN_PARDON_STATUS))
                <input class="remove-hr" type="button" value="{{ $lang_myhr['action_remove'] ?? '' }}" data-id="{{ $row->id }}">
            @endif
        </td>
    </tr>
    @endforeach
