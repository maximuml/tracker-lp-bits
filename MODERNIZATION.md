...
|------|--------|-------|--------|
| 1.1 | Re-enable CSRF: replace blanket `$except` with per-route tokens or signed-form middleware. Keep only `tg-webhook/*` exempt. | `app/Http/Middleware/VerifyCsrfToken.php`, all legacy form views | High — **DONE** |
| 1.2 | Protect `/cron`: require `CRON_TOKEN` query param or restrict to loopback via middleware. | `routes/legacy/public.php`, new `CronToken` middleware | Low — **DONE** |
