...
if (! isset($lang_shoutbox)) {
    $lang_shoutbox = (array) (SupportContext::getGlobal('lang_shoutbox') ?? []);
}
// Delete action is handled by ShoutboxController before rendering.
$isAjax = $isAjax ?? ! empty(SupportContext::getQuery('ajax'));
...
<link rel="stylesheet" href="<?php echo Style::cssUriWithContext().'theme.css'?>" type="text/css">
<link rel="stylesheet" href="styles/curtain_imageresizer.css" type="text/css">
<link rel="stylesheet" href="styles/nexus.css" type="text/css">
<script src="js/curtain_imageresizer.js" type="text/javascript"></script><script>var SHOUT_CSRF = '<?php echo htmlspecialchars(Shoutbox::csrfToken((int) ($CURUSER['id'] ?? 0))); ?>';</script><script src="js/shoutbox.js" type="text/javascript"></script><link rel="stylesheet" href="styles/shoutbox.css" type="text/css">
<?php
    echo Style::addiCodeWithContext();
 ...
 */
if (! function_exists('shoutbox_class_badge')) {
    function shoutbox_class_badge(int $class): string
    {
        static $map = null;
        if ($map === null) {
            $map = [
...
if ($rows->isEmpty()) {
    ...
    foreach ($rows as $arr) {
        ...
        if (! empty($arr['edited_at']) && (int) $arr['edited_at'] > 0) {
            $editedNote = ' <span class="shout-edited-note">('.htmlspecialchars((string) ($lang_shoutbox['text_edited'] ?? 'edited')).' '.Shoutbox::formatTime((int) $arr['edited_at'], true).')</span>';
        }
        $avatarUrl = 'pic/default_avatar.png';
        $nickReplyName = '';
        if ($arr['userid']) {
            $username = UserDisplay::username($arr['userid'], false, true, true, true, false, false, '', true);
            $userRow = UserDisplay::row((int) $arr['userid']);
        ...
        $mentionsMe = false;
        $message = Shoutbox::formatMessage($arr['text'], $currentUserId, $mentionsMe);
        ...
        echo '<tr><td class="'.$rowClass."\"><span class='date'>[".$time.']</span> '.
