...
$searchBoxId = 1;
$torrentGridRows = function (\Illuminate\Database\Eloquent\Collection $torrents) use ($searchBoxId): array {
    return array_map(
        fn ($torrent) => ['search_box_id' => $searchBoxId] + $torrent->toArray(),
        $torrents->all()
    );
};
$latestRows = $torrentGridRows($latest);
$trendingRows = $torrentGridRows($trending);
$snatchedRows = $torrentGridRows($snatched);
...
<section class="i2-section i2-torrents">
    <?php if ($showlastxtorrents_main == "yes") { ?>
        ...
        <?php echo \App\Support\TorrentGrid::render($latestRows, 'card', $searchBoxId, true); ?>
    <?php } ?>

    <h2 class="i2-card-grid-title"><?php echo htmlspecialchars($lang_index['text_trending'] ?? 'Trending') ?></h2>
    <?php echo \App\Support\TorrentGrid::render($trendingRows, 'card', $searchBoxId, true); ?>

    <h2 class="i2-card-grid-title"><?php echo htmlspecialchars($lang_index['text_most_snatched'] ?? 'Most Snatched') ?></h2>
    <?php echo \App\Support\TorrentGrid::render($snatchedRows, 'card', $searchBoxId, true); ?>
</section>
