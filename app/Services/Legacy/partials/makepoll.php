...
if (! isset($lang_makepoll)) {
    $lang_makepoll = (array) (SupportContext::getGlobal('lang_makepoll') ?? []);
}
...
if ($pollid > 0) {
    echo '<h1>'.($lang_makepoll['text_edit_poll'] ?? 'Edit poll').'</h1>';
} else {
    if ($ageWarning !== '') {
        echo '<p><font class=striking><b>'.$ageWarning.'</b></font></p>';
    }
    echo '<h1>'.($lang_makepoll['text_make_poll'] ?? 'Make poll').'</h1>';
}
?>
