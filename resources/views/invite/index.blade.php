...
@if ($type == 'new')
    ...
    <div class="nx-fgrid">
    <div class="nx-ffull nx-center"><b>{{ $lang_invite['text_invite_someone'] ?? '' }}{{ $SITENAME }} ({{ $inv['invites'] ?? 0 }}{{ $lang_invite['text_invitation'] ?? '' }}{{ $_s }}{{ $lang_invite['text_left'] ?? '' }} + {{ sprintf($lang_invite['text_temporary_left'] ?? '%d', count($temporaryInvites)) }})</b></div>
    <div class="nx-fhead nx-nowrap">{{ $lang_invite['text_email_address'] ?? '' }}</div><div class="nx-fcell"><input type=text size=40 name=email><br /><font align=left class=small>@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml($lang_invite['text_email_address_note'] ?? ''))</font></div>
    @safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml($preUsernameTr))
    <div class="nx-fhead nx-nowrap">{{ $lang_invite['text_consume_invite'] ?? '' }}</div><div class="nx-fcell"><select name='hash'>@safeHtml(\App\Support\Html\SafeHtml::fromTrustedHtml($inviteSelectOptions))</select></div>
    <div class="nx-fhead nx-nowrap">{{ $lang_invite['text_message'] ?? '' }}</div><div class="nx-fcell"><textarea name=body rows=10 style='width: 100%'>{{ $invitation_body }}</textarea></div>
    <div class="nx-ffull nx-center"><input type=submit value='{{ $lang_invite['submit_invite'] ?? '' }}'></div>
    </form></div></div>
...
@else
    ...
    @if ($menuSelected == 'invitee')
        ...
        @if ($canConfirm)
            @if ($pendingCount)
                <tr><td colspan={{ $inviteeColSpan }} align=right><input type=submit style='height: 20px' value='{{ $lang_invite['submit_confirm_users'] ?? '' }}'></td></tr>
            @endif
            </form>
        @endif
        </table>
