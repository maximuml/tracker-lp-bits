...
<b>'.htmlspecialchars($lang_usercp['text_comments_reviews'] ?? '').': </b><br /><input type=checkbox name=showcomnum '.(($CURUSER['showcomnum'] ?? '') === 'yes' ? ' checked' : '').' value=yes>'.($lang_usercp['text_show_comment_number'] ?? '').($showtooltipsetting ? '<select name="showlastcom" style="width: 70px;"><option value="yes" '.(($CURUSER['showlastcom'] ?? 'no') !== 'no' ? ' selected' : '').'>'.htmlspecialchars($lang_usercp['select_with'] ?? '').'</option><option value="no" '.(($CURUSER['showlastcom'] ?? '') === 'no' ? ' selected' : '').'>'.htmlspecialchars($lang_usercp['select_without'] ?? '').'</option></select>'.($lang_usercp['text_last_comment_on_tooltip'] ?? '') : '');
