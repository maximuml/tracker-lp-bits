...
if (! isset($lang_messages)) {
    $lang_messages = (array) (SupportContext::getGlobal('lang_messages') ?? []);
}
// Define constants
...
if (! defined('PM_INBOX')) {
    define('PM_INBOX', 1);
} // Message located in Inbox for reciever
if (! defined('PM_SENTBOX')) {
    define('PM_SENTBOX', -1);
} // GET value for sent box
...
if (! function_exists('insertJumpTo')) {
    function insertJumpTo($selected = 0)
    {
        ...
        $pmBoxes = MessageRepository::getUserMailboxes((int) $CURUSER['id']);
        $place = SupportContext::getQuery('place') ?? '';
        ?>
<form action="messages.php" method="get">
<input type="hidden" name="action" value="viewmailbox"><?php echo $lang_messages['text_search'] ?>&nbsp;&nbsp;<input id="searchinput" name="keyword" type="text" value="<?php echo htmlspecialchars(SupportContext::getQuery('keyword') ?? '')?>" style="width: 200px"/>
...
if (! function_exists('messagemenu')) {
    function messagemenu($selected = 1)
    {
        $lang_messages = (array) (SupportContext::getGlobal('lang_messages') ?? []);
        $BASEURL = SupportContext::getGlobal('BASEURL');
        $CURUSER = SupportContext::getUser() ?? [];
        echo '<div id="pmboxnav"><ul id="pmboxmenu" class="menu">';
        echo '<li'.($selected == 1 ? ' class=selected' : '').'><a href="'.Http::protocolPrefix(Url::isSecure()).$BASEURL.'/messages.php" >'.$lang_messages['text_inbox'].'</a></li>';
        echo '<li'.($selected == -1 ? ' class=selected' : '').'><a href="'.Http::protocolPrefix(Url::isSecure()).$BASEURL.'/messages.php?action=viewmailbox&box=-1">'.$lang_messages['text_sentbox'].'</a></li>';
        $pmBoxes = MessageRepository::getUserMailboxes((int) $CURUSER['id']);
        if ($pmBoxes->count()) {
            foreach ($pmBoxes as $row) {
                $row = (array) $row;
                echo '<li'.($selected == $row['boxnumber'] ? ' class=selected' : '').'><a href="'.Http::protocolPrefix(Url::isSecure()).$BASEURL.'/messages.php?action=viewmailbox&box='.$row['boxnumber'].'">'.$row['name'].'</a></li>';
            }
        }
        echo '</ul></div>';
    }
...
if (! $action) {
    $action = SupportContext::getPost('action') ?? '';
    if (! $action) {
        $action = 'viewmailbox';
    }
}
...
if ($action == 'viewmailbox') {
    // Get Mailbox Number
    $mailbox = SupportContext::getQuery('box') ?? 0;
    if (! $mailbox) {
        $mailbox = PM_INBOX;
    }
    ...
    if ($mailbox != PM_INBOX && $mailbox != PM_SENTBOX) {
        $pmBoxName = MessageRepository::getMailboxName((int) $CURUSER['id'], (int) $mailbox);
        if (! $pmBoxName) {
            LegacyResponse::abort($lang_messages['std_error'], $lang_messages['std_invalid_mailbox']);
        }
...
<?php
    // search
    $keyword = trim((string) (SupportContext::getQuery('keyword') ?? ''));
    $place = (string) (SupportContext::getQuery('place') ?? '');
    $unread = SupportContext::getQuery('unread') ?? '';
    $perpage = ($CURUSER['pmnum'] ? (int) $CURUSER['pmnum'] : 20);

    $count = MessageRepository::getMailboxMessages((int) $CURUSER['id'], (int) $mailbox, $keyword, $place, is_string($unread) ? $unread : null, 0, 0)['count'];

    [$pagertop, $pagerbottom, , $offset, $perpage] = Pagination::pager($perpage, $count, '?action=viewmailbox'.($mailbox ? '&box='.$mailbox : '').($place ? '&place='.$place : '').($keyword ? '&keyword='.rawurlencode($keyword) : '').($unread ? '&unread='.$unread : '').'&');

    $messageResult = MessageRepository::getMailboxMessages((int) $CURUSER['id'], (int) $mailbox, $keyword, $place, is_string($unread) ? $unread : null, (int) $offset, (int) $perpage);
...
<?php
        echo "<td width=\"35%\" class=\"colhead\" align=\"left\">$sender_receiver</td>";
        ?>
...
foreach ($messages as $message) {
    ...
    if ($row['sender'] != 0) {
        if ($mailbox != PM_SENTBOX) {
            $username = UserDisplay::username($row['sender']);
        } else {
            $username = UserDisplay::username($row['receiver']);
        }
    } else {
    ...
    echo '<td class=rowfollow nowrap>'.Time::format($row['added'], true, false)."</td>\n";
...
<?php if ($mailbox != PM_SENTBOX) {
    echo '<input class=btn type="submit" name="markread" value="'.$lang_messages['submit_mark_as_read'].'">';
} ?>
<input class=btn type="submit" name="delete" value=<?php echo $lang_messages['submit_delete']?>>
<?php
if ($mailbox != PM_SENTBOX) {
    echo $lang_messages['text_or'];
    echo '<input class=btn type="submit" name="move" value="'.$lang_messages['submit_move_to'].'"> <select name="box"><option value="1">'.$lang_messages['text_inbox'].'</option>';
    $pmBoxes = MessageRepository::getUserMailboxes((int) $CURUSER['id']);
    foreach ($pmBoxes as $row) {
        $row = (array) $row;
...
if ($action == 'viewmessage') {
    $pm_id = (int) SupportContext::getQuery('id');
    if (! $pm_id) {
        LegacyResponse::abort($lang_messages['std_error'], $lang_messages['std_no_permission']);
    }

    // Get the message
    $messageModel = MessageRepository::getMessageForUser($pm_id, (int) $CURUSER['id']);
    $message = $messageModel->toArray();
    // Prepare for displaying message
    if ($message['sender'] == $CURUSER['id']) {
        // Display to
        $sender = UserDisplay::username($message['receiver']);
        $reply = '';
    ...
    // Mark message unread
    MessageRepository::markAsRead($pm_id, (int) $CURUSER['id']);
...
<td class="rowfollow" align="left"><?php echo Time::format($added, true, false)?>&nbsp;&nbsp;<?php echo $unread?></td>
</tr>
<tr>
...
<?php if ($message['sender'] != $CURUSER['id']) {
    echo '<form action="messages.php" method="post">'.csrf_field().'<input type="hidden" name="action" value="moveordel"><input type="hidden" name="id" value='.$pm_id.'>
...
if ($action == 'forward') {
    ...
    // Get the message
    $messageModel = MessageRepository::getMessageForForward($pm_id, (int) $CURUSER['id']);
    if (! $messageModel) {
        LegacyResponse::abort($lang_messages['std_error'], $lang_messages['std_no_permission_forwarding']);
    }
    $message = $messageModel->toArray();
    ...
    $from_name = UserDisplay::username($from);
    if ($orig == 0) {
        $orig_name = $orig_name2 = $lang_messages['text_system'];
    } else {
        $orig_name = UserDisplay::username($orig);
        $orig_name2 = MessageRepository::getUsername((int) $orig) ?? '';
    }

    $body = '-------- Original Message from '.$orig_name2.' --------<br />'.Format::formatComment($message['msg']);
